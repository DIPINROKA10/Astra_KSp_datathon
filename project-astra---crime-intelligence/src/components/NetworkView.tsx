  ...
  return (
    <div className="flex-1 bg-[#090b11] h-screen flex flex-col font-sans text-slate-300 overflow-hidden relative">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Graph Canvas */}
        <div className="flex-1 bg-[#07090e] relative flex items-center justify-center p-4 md:p-8 select-none overflow-hidden">
          
          {/* Zoom controls */}
          <div className="absolute top-3 left-3 md:top-6 md:left-6 flex items-center gap-1 md:gap-1.5 p-1 md:p-1.5 bg-[#0c0e17] border border-slate-800 rounded-lg z-10 shadow-md">
            <button 
              onClick={() => setPhysicsOffset({ x: physicsOffset.x - 30, y: physicsOffset.y })}
              className="p-1 md:p-1.5 hover:bg-slate-800 rounded text-slate-400 hover:text-white"
            >
              ←
            </button>
            <button 
            ...
            <span className="w-px h-3 md:h-4 bg-slate-800" />
            <button 
              onClick={() => setPhysicsOffset({ x: 0, y: 0 })}
              className="px-1.5 md:px-2 py-0.5 text-[9px] md:text-[10px] font-mono text-slate-400 hover:text-white hover:bg-slate-800 rounded"
            >
              Reset
            </button>
          </div>
          ...
          <div className="absolute top-3 right-3 md:top-6 md:right-6 flex items-center gap-0.5 md:gap-1.5 bg-[#0c0e17] p-0.5 md:p-1 rounded-lg border border-[#1e293b]/50 z-10 overflow-x-auto">
              ...
              return (
                ...
                >
                  <div className={`flex flex-col items-center`}>
                    <div className={`p-2 md:p-3 rounded-xl border-2 shadow-xl ${style.bg} ${
                      isSelected ? 'ring-4 ring-blue-500/30 border-blue-400 scale-105' : ''
                    }`}>
                      <Icon className="w-4 h-4 md:w-5 md:h-5" />
                    </div>
                    
                    <span className="mt-1 md:mt-1.5 px-1.5 md:px-2 py-0.5 rounded-md bg-[#0a0d14]/90 border border-slate-800 text-[8px] md:text-[10px] font-semibold text-slate-300 font-sans tracking-tight max-w-[80px] md:max-w-[120px] truncate">
                      {node.label}
                    </span>
                  </div>
                </div>
          ...
          {/* Legend */}
          <div className="absolute bottom-3 left-3 md:bottom-6 md:left-6 p-3 md:p-4 bg-[#0c0e17] border border-slate-800 rounded-lg shadow-xl z-10 w-40 md:w-52 font-sans">
            <span className="text-[8px] md:text-[9px] font-bold text-slate-400 font-mono tracking-wider uppercase block mb-2">Graph Legend</span>
            <div className="space-y-1.5 md:space-y-2 text-[10px] md:text-xs">
              <div className="flex items-center gap-2">
                <span className="w-3 md:w-4 h-0.5 bg-slate-500 inline-block" />
                <span className="text-slate-400 font-mono text-[10px] md:text-[11px]">Confirmed</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 md:w-4 h-0.5 border-t-2 border-dashed border-[#a78bfa] inline-block" />
                <span className="text-[#a78bfa] font-mono text-[10px] md:text-[11px] font-semibold">CrimeGAT Link</span>
              </div>
            </div>
          </div>
        </div>

        {/* Selected Node Properties - Overlay on mobile, side panel on desktop */}
        {selectedNode && (
          <>
            ...
            />
            <div className="fixed md:relative right-0 top-0 bottom-0 w-80 max-w-[85vw] bg-[#0c0e17] border-l border-[#1e293b]/50 h-full overflow-y-auto p-4 md:p-6 flex flex-col z-40 shadow-2xl relative font-sans">
              <div className="flex items-center justify-between md:justify-start pb-4 border-b border-slate-800/80 mb-5">
                <div className="flex items-center gap-2">
              ...
              <div className="flex-1 space-y-3 md:space-y-4 text-[10px] md:text-xs">
                  ...
                  <div className="space-y-3">
                    {selectedNode.properties.offender_score !== undefined && (
                      <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 text-center">
                        <span className="text-[9px] md:text-[10px] font-mono text-red-400 block tracking-wider uppercase font-bold mb-0.5">Composite Offender Score</span>
                    ...
                    {selectedNode.properties.address && (
                      <div>
                        <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Primary Residence</span>
                    ...
                    {selectedNode.properties.district && (
                      <div>
                        <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Assigned District</span>
                ...
                {selectedNode.type === 'OBJECT' && (
                  <div className="space-y-3">
                    <div>
                      <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Registration Number</span>
                      <span className="text-slate-300 font-mono font-bold text-xs md:text-sm bg-slate-900 border border-slate-800 px-2 py-0.5 rounded inline-block">{selectedNode.properties.reg_number}</span>
                    </div>
                    <div>
                      <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Vehicle Model</span>
                ...
                {selectedNode.type === 'LOCATION' && (
                  <div className="space-y-3">
                    <div>
                      <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Geocoded Coordinates</span>
                    ...
                    {selectedNode.properties.h3_index && (
                      <div>
                        <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">H3 Grid Hex Index</span>
                        <span className="text-slate-300 font-mono font-bold bg-slate-900 border border-slate-800 px-2 py-0.5 rounded inline-block text-[10px]">{selectedNode.properties.h3_index}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {selectedNode.type === 'EVENT' && (
                  <div className="space-y-3">
                    <div>
                      <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Incident Date</span>
                      <span className="text-slate-300 font-sans">{selectedNode.properties.date}</span>
                    </div>
                    <div>
                      <span className="text-[9px] md:text-[10px] font-mono text-slate-500 uppercase block mb-0.5">Statute Classifications</span>
                      <span className="text-blue-400 font-mono block mt-0.5 font-semibold">{selectedNode.properties.bns_sections?.join(', ')}</span>
                    </div>
                  </div>
                )}

                <div className="pt-3 md:pt-4 border-t border-slate-800/80">
                  <div className="flex items-center gap-1.5 text-[#a78bfa] mb-2">
                    <Lock className="w-3.5 h-3.5" />
                    <span className="text-[9px] md:text-[10px] font-mono font-bold tracking-wider uppercase">CrimeGAT Inference</span>
                  </div>
                  ...
                  {mockEdges.filter(e => e.predicted && (e.source === selectedNode.id || e.target === selectedNode.id)).length > 0 ? (
                    <div className="space-y-2 bg-[#14182b] rounded-lg p-3 border border-purple-500/10">
                      <p className="text-[10px] md:text-[11px] text-slate-400 leading-relaxed">
                        AI Graph models predicted a high probability linking this suspect to crime rings:
                      </p>
                        ...
                        {mockEdges.filter(e => e.predicted && (e.source === selectedNode.id || e.target === selectedNode.id)).map((edge, i) => {
                          ...
                          return (
                            <div key={i} className="flex items-center justify-between text-[10px] md:text-[11px]">
                              <span className="text-slate-200 font-medium truncate mr-2">Link with {targetNode?.label}</span>
                              <span className="text-[#a78bfa] font-mono font-bold shrink-0">P = {Math.round(edge.weight * 100)}%</span>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <p className="text-[9px] md:text-[10px] text-slate-500 leading-relaxed font-mono">
                      No predicted GAT links matching high risk thresholds.
                    </p>
                  )}
                </div>
              </div>

              <div className="pt-4 md:pt-6 border-t border-slate-800/80 flex items-center gap-2">
                <button 
                ...
                >
                  ...
                  <span>{expandedNodes.includes(selectedNode.id) ? 'Collapse' : 'Expand Network'}</span>
                </button>
              </div>
            </div>
          </>
